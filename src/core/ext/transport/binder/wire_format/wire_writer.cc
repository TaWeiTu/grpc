...
const size_t WireWriterImpl::kBlockSize = 16 * 1024;
const size_t WireWriterImpl::kFlowControlWindowSize = 128 * 1024;
...
absl::Status WireWriterImpl::RpcCall(const Transaction& tx) {
  ...
  while (ptr < data.size()) {
    ...
    size_t size = std::min(kBlockSize, data.size() - ptr);
...
absl::Status WireWriterImpl::SendAck(size_t num_bytes) {
  grpc_core::MutexLock lock(&mu_);
  RETURN_IF_ERROR(binder_->PrepareTransaction());
  WritableParcel* parcel = binder_->GetWritableParcel();
  RETURN_IF_ERROR(parcel->WriteInt64(static_cast<int64_t>(num_bytes)));
  return binder_->Transact(BinderTransportTxCode::ACKNOWLEDGE_BYTES);
}

void WireWriterImpl::RecvAck(size_t num_bytes) {
  grpc_core::MutexLock lock(&mu_);
