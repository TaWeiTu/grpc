...
TEST_P(End2EndBinderTransportTest, UnaryCallServerTimeout) {
  ...
  // Server will sleep for 5 seconds before responding us.
...
TEST_P(End2EndBinderTransportTest, ServerStreamingCallTryCancelBeforeProcessing) {
  ...
  while (reader->Read(&response)) {
    EXPECT_EQ(
        response.message(),
        "ServerStreamingCallTryCancelBeforeProcessing" + std::to_string(cnt));
    cnt++;
  }
  EXPECT_EQ(cnt, 0);
  grpc::Status status = reader->Finish();
...
TEST_P(End2EndBinderTransportTest,
  ...
  for (size_t i = 0; i < kClientStreamingCounts; ++i) {
    grpc::testing::EchoRequest request;
    request.set_message(absl::StrFormat("it works!(%d)", i));
...
TEST_P(End2EndBinderTransportTest, BiDirStreamingCallThroughFakeBinderChannel) {
  ...
  std::shared_ptr<grpc::ClientReaderWriter<grpc::testing::EchoRequest,
      ...
      stream = stub->EchoBiDirStreamingCall(&context);
  ...
  for (size_t i = 0; i < kBiDirStreamingCounts; ++i) {
    ...
    EXPECT_EQ(response.message(), absl::StrFormat("it works!(%d)", i));
  }
